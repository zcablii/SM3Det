...
def pic(model,savedir,filename,filedir,temp):
    ...
    for tt in range(0,len(ls)):
        id=ls[tt]
        experts_ids=torch.load(temp+f'/expert_id_{id}.pt')
        # print(i)
        print(experts_ids.shape)
        for jj in range(0,6):
            # print(i)
            experts_id=experts_ids[jj]
            # print(experts_id)
            ...
            color = np.zeros((expert_num+1,img.shape[0]//experts_id.shape[0]-1,img.shape[1]//experts_id.shape[1]-1,3),dtype=np.uint8)
            for t in range(0,expert_num+1):
                # print(Color)
                # print(i)
                # print(t)
            ...
            cv2.imwrite(f'{path1}/0.7/{imgname}_expert_{id}_{jj+1}.jpg', img1)
            cv2.imwrite(f'{path1}/0.3/{imgname}_expert_{id}_{jj+1}.jpg', img2)
            cv2.imwrite(f'{path2}/{imgname}_expert_{id}_{jj+1}.jpg', img3)
def main(args):
