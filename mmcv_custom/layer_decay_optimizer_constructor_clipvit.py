...
def get_num_layer_for_vit(var_name, num_max_layer):
    if var_name =='backbone.level_embed':
        return 0
    elif var_name.startswith('backbone.model.conv1') or \
            var_name.startswith('backbone.model.positional_embedding') or \
            var_name.startswith('backbone.model.ln_pre') or \
            var_name.startswith('backbone.model.patch_dropout'):
        return 0
    elif var_name.startswith('backbone.model.transformer.resblocks'):
...
class CLIPViTLayerDecayOptimizerConstructor(DefaultOptimizerConstructor):
