...
from transformers import (AutoConfig, AutoModelForCausalLM, AutoTokenizer,
                          HfArgumentParser, Trainer, TrainerCallback, TrainingArguments,
                          set_seed)
...
class UpdateStepCallback(TrainerCallback):
    def on_step_begin(self, args, state, control, model=None, **kwargs):
        if hasattr(model, 'step'):
            model.step = state.global_step
        elif hasattr(model, 'module') and hasattr(model.module, 'step'):
            model.module.step = state.global_step
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        max_num_frame=16,  # for video data
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        config.select_layer = ast.literal_eval(model_args.vision_select_layer)
        ...
        internvl_chat_config = InternVLChatConfig(
            ...
            select_layer=ast.literal_eval(model_args.vision_select_layer), dynamic_image_size=data_args.dynamic_image_size,
